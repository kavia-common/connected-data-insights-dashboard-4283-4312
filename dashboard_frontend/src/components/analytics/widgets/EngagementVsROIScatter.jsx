 ...
 */
const EngagementVsROIScatter = () => {
  const { filteredRows } = useMarketData();
  const { data, corr } = useMemo(() => {
    const pts = filteredRows
      .filter(r => Number.isFinite(r.Engagement_Score) && Number.isFinite(r.ROI))
      .map(r => ({ x: r.Engagement_Score, y: r.ROI }));
  ...
  return (
    <div className="dashboard-card" style={{ padding: 16 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8, alignItems: 'baseline' }}>
        <h3 style={{ margin: 0 }}>Engagement vs ROI</h3>
      ...
      <div style={{ width: '100%', height: 280 }}>
        <ResponsiveContainer>
          <ScatterChart margin={{ top: 8, right: 16, bottom: 8, left: 0 }}>
            ...
            <XAxis dataKey="x" name="Engagement" tick={{ fontSize: 12 }} />
...
export default EngagementVsROIScatter;

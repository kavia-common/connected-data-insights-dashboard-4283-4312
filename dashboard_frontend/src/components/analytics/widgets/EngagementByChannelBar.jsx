...
const EngagementByChannelBar = () => {
  ...
  const data = useMemo(() => {
    ...
    filteredRows.forEach(r => {
      ...
      if (!m.has(c)) m.set(c, []);
      if (Number.isFinite(r.Engagement_Score)) m.get(c).push(r.Engagement_Score);
    });
    return Array.from(m.entries()).map(([Channel, arr]) => ({
      Channel,
      'Avg Engagement': arr.length ? arr.reduce((a,b)=>a+b,0)/arr.length : 0
    }));
  }, [filteredRows]);
  ...
  return (
    <div className="dashboard-card" style={{ padding: 16 }}>
      <div style={{display:'flex', justifyContent:'space-between', marginBottom:8}}>
        <h3 style={{ margin: 0 }}>Engagement by Channel</h3>
...
export default EngagementByChannelBar;
